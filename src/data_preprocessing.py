"""Feature engineering and inference preprocessing for demand forecasting."""
...
def fit_encoders(data: pd.DataFrame, output_path: Path = ENCODERS_PATH) -> dict[str, Any]:
    """Fit the existing encoder dictionary format used by inference."""
...
def prepare_data(
    ...
    """Run the complete training preprocessing pipeline and save its artifacts."""
    ...
    tracking_columns = ["Store ID", "Product ID", "Date", "forecast_period", "forecast_horizon"]
    output = prepared[tracking_columns + feature_columns + ["Target_Demand"]]
...
def preprocess_input(
    ...
    history_df: pd.DataFrame | Path | None = None,
    encoders_path: Path = ENCODERS_PATH,
    feature_columns_path: Path = FEATURE_COLUMNS_PATH,
) -> np.ndarray:
    """Transform one API row using the same features and order as training."""
    # Preserve compatibility with the old preprocess_input(row, encoders_path) call.
    if isinstance(history_df, (str, Path)):
        encoders_path = Path(history_df)
        history_df = None

    encoders = _load_encoders(encoders_path)
    feature_columns = _load_pickle(feature_columns_path) if feature_columns_path.exists() else encoders["feature_order"]
    row = {dataset_name: input_data[api_name] for api_name, dataset_name in INPUT_MAPPING.items() if api_name in input_data}
    ...
    if "forecast_period" in input_data:
        period = input_data["forecast_period"]
    ...
    elif "forecast_horizon" in input_data:
        try:
            horizon = int(input_data["forecast_horizon"])
        except (TypeError, ValueError) as error:
    ...
    if history_df is None:
        raise ValueError("history_df is required to calculate lag and rolling features.")
    history = history_df.copy()
    if not set([*GROUP_COLUMNS, "Date", "Units Sold"]).issubset(history.columns):
        raise ValueError("history_df must contain Store ID, Product ID, Date, and Units Sold.")
    history = history[
    ...
    return pd.DataFrame([[values[column] for column in feature_columns]], columns=feature_columns).astype(float).to_numpy()
